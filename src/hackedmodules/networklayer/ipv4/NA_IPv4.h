 ...
 *
 */
class INET_API NA_IPv4 : public IPv4, public NA_HackedModule {
...
private:
    ...
    //Detection flag for (de)activation
    bool droppingSADetectionIsActive;

    //Detection controller pointer, used for event-based window trigger callback
    NA_Detector *detectionSAController;

    //Event-based window size
    long eventWindowSize;

    //Restart the detection counters
...
protected:
    ...
    //Override function for count fordwarded packets
    void routeUnicastPacket(IPv4Datagram *datagram, const InterfaceEntry *fromIE, const InterfaceEntry *destIE, IPv4Address requestedNextHopAddress);
    void sendDatagramToOutput(IPv4Datagram *datagram, const InterfaceEntry *ie, IPv4Address nextHopAddr);
