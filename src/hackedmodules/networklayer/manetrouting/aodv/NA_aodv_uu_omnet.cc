...
void NS_CLASS initialize(int stage)
{
    ...
    if (stage==4)
    {
        ...
        //Dropping Detection initialization
...
void NS_CLASS restartDetectionCounters(){
    LOG << "NA_AODVUU: Restarting detection counters\n";
    RREQCount = 0;
}

void NS_CLASS handleMessageFromDetectionController(cMessage *msg, NA_Detector *controller){
    // It is necessary to call Enter_Method for doing context switching (4.10 of User Manual)
        Enter_Method
        ("NA_AODVUU: handle message from detection controller");
        ...
        if (not strcmp(msg->getFullName(), "droppingSAActivate")) {
            ...
            NA_DroppingDetectorStandaloneMessage *data =
                    new NA_DroppingDetectorStandaloneMessage("droppingSAAODVUUData");
            data->setRREQCount(RREQCount);
